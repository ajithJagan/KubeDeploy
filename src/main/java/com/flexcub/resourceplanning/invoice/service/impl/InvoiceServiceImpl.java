        ...
        try {
            ...
            if (partnerGenerated) {
                ...
                if (invoiceEntity.isPresent()) {
                    ...
                    invoiceDetailResponse.setSubmittedDate(date);
                    ...
                    c.setTime(invoiceDetailResponse.getSubmittedDate());
                    c.add(Calendar.DAY_OF_WEEK, 41);
                    invoiceDetailResponse.setDueDate(c.getTime());
                ...
                if (invoiceEntity.isPresent()) {
                    logger.info("InvoiceServiceImpl || getData || Get Details for particular Invoice Id ");
                    InvoiceDetailResponse invoiceDetailResponse = new InvoiceDetailResponse();
//                    invoiceDetailResponse.setDueDate(Date.from(invoiceEntity.get().getDueDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
                    invoiceDetailResponse.setSubmittedDate(date);
        ...
        if (!invoiceDataValue.isEmpty()) {
            for (InvoiceData invoiceData : invoiceDataValue) {
                ...
                invoices.forEach(invoice -> {
                    invoice.getInvoiceData().forEach(invoiceDataValues -> {
                        if (invoiceDataValues.getId() == invoiceData.getId()) {
                            ...
                            adminInvoice.setRateCard(skillOwnerRepository.findBySkillOwnerEntityId(invoiceData.getOwnerId().getSkillOwnerEntityId()).getRateCard());
