...
class ClientServiceImplTest {
    ...
    public void setup() {
        ...
        client.setStartDate(new Date(2022 - 11 - 12));
        client.setEndDate(new Date(2022 - 12 - 12));
    ...
    void insertClientTest() {
        when(skillOwnerRepository.findById(client.getSkillOwnerEntityId())).thenReturn(Optional.ofNullable(skillOwnerEntity));
        when(modelMapper.map(Mockito.any(), Mockito.any())).thenReturn(client);
        assertThat(clientService.insertClient(client)).isEqualTo(client);
    }
