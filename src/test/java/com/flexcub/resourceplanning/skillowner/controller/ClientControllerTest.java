...
class ClientControllerTest {
    ...
    void insertClientTest() {
        Mockito.when(clientService.insertClient(client)).thenReturn(client);
        assertEquals(200, clientController.insertClient(client).getStatusCodeValue());
        assertEquals(clientController.insertClient(client).getBody().getClientId(), client.getClientId());
    }
