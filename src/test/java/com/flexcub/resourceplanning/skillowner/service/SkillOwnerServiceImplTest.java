...
//        assertEquals(skillOwnerEntity, skillOwnerService.insertData(skillOwnerEntities).get(0));
    ...
    void updateOwnerProfileTest() {
        ...
        Mockito.when(skillOwnerPortfolioRepo.saveAndFlush(skillOwnerPortfolio)).thenReturn(skillOwnerPortfolio);
        when(skillOwnerRepository.save(skillOwnerEntity)).thenReturn(skillOwnerEntity);
        assertEquals(ownerDto.getPortfolioUrl(), skillOwnerService.updateOwnerProfile(skillOwnerEntity).getPortfolioUrl());
